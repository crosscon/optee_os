...
static inline void plic_set_priority(struct plic_data *pd, uint32_t source,
				     uint32_t priority)
{
...
static inline void plic_complete_interrupt(struct plic_data *pd,
					   uint32_t source)
{
	uint32_t hartid = get_core_pos();
